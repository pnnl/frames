...
C                           1 June 2010  BAN Minor revision to animal drinking water source logic
C
C-----------------------------------------------------------------------
C
...
C     Process each selected terrestrial food type--                     
      ...
      IF (TFOOD) THEN
        DO 101 ITF = 1, NTF   ! LV, OV, Fruit, Grain
          IF (TFD(ITF)) THEN
           IF (H3) THEN
...
c                    WATCON(1) = SWCON(1)
c                  ELSEIF (IRRST(ITF) .EQ. 1) THEN
c                    WATCON(1) = GWCON(1)
c                  ELSE 
c                   WATCON(1) = 0.0
c  revise this to behave more like other routines  BAN 6/1/2010
                 WATCON(1) = 0.0
                 IF (IRRSA(ITF) .EQ. 1) THEN
                   WATCON(1) = GWCON(1)
                 ELSE
                   WATCON(1) = SWCON(1)
                 ENDIF
...
C             HTO-OBT treated as chain with zero branching ratio
              T = HLDUP(ITF) * YRDA
...
c  revised 1 June 2010 BAN
c                  IF (IRRSA(IAN) .EQ. 2) THEN
c                    WATCON(1) = SWCON(1)
c                  ELSEIF (IRRSA(IAN) .EQ. 1) THEN
c                    WATCON(1) = GWCON(1)
c                  ELSE 
c                    WATCON(1) = 0.0
c                  ENDIF
...
C                 Calculate leaf deposition rate from irrigation--
                  LEAF2 = 0.0
                  IF(IRRSA(IAN) .GT. 0.0 .AND. IRTIMA(IAN) .GT. 0.0)THEN
