...
C
      REAL*8 A(CHAINLEN *(CHAINLEN-1)/2+CHAINLEN),
     .       SUMPR, ASUM, AMD(CHAINLEN), EXPO(CHAINLEN),
     .       ABD(CHAINLEN), EX2(CHAINLEN)
      REAL*8 ARG, TERM
      DIMENSION AM(CHAINLEN), AO(CHAINLEN), AB(CHAINLEN)                                                           
c      
C-----  Divide by lambda to convert to atom equivalent units -----------------
c
      DO 113 IJK = 1, CHAINLEN
        EXPO(IJK) = 0.0D0
        EX2(IJK) = 0.0D0
...
C------SERIES EXPANSION OF EXPONENTIAL TERM ----
         ELSE
           ...
           DO 14  IT = 2, I
             TERM = TERM * ARG / ABD(J)
             EX2(J) = EX2(J) + TERM / ABD(J)
   14      CONTINUE
           EXPO(J) = (T - EX2(J))/ABD(J)
         ENDIF
        ENDIF
C
