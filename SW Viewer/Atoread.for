...
c
	...
	READ(NATO,*)DUM, ANUKE,NTIMES,NPROG
...
C
      WRITE(NEPA,111)
  111 FORMAT(/,'MIRROR OF ATMOSPHERIC TRANSPORT RESULT (FROM ATO FILE)')
	READ(NATO,*)NASETS
      DO ISET = 1, NASETS
	READ(NATO,*)NGP, DUM
	DO I = 1,NGP
...
C
  ...
  115   FORMAT('TIME PERIOD',F4.1,1X,A7)
C
...
c
  ...
  120       FORMAT('TIME PERIOD',F4.1,1X,A7)
            DO IO = 1,NOPS
              READ(NATO,*) ONAME,OTYPE,WETDRY,OU,NAX1,CH,NAX2,AU
              WRITE(NEPA,119) CNAMIN,CIDIN,NTMP
			WRITE(NEPA,116) ONAME,OTYPE,WETDRY,OU,NAX1,CH,NAX2,AU
