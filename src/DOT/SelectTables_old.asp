...
<%	
    strSQL = "Select count(*)as KeyCount,KeyName From tblParamKeys K Where K.ParameterID = " & Request("ExtPlanParam") & "Group by KeyName"
	rsObject.Open strSQL,objCon%>
    
<%	If not rsObject.EOF Then%>
		<FORM name=DTForm method="post" action="SelectFields.asp">
		<% KCount = rsObject.Fields("KeyCount").Value %>
<%	Else%>
...
<%	rsObject.Close
	...
	<P>Select the table(s) for the Extraction Plan</P>
	
		<TABLE width=50% border=no align=center>
<%		Count = 1
