...
c    6 Sept 12 BAN  resized PFilNam to 80
c
C
C==== SUBROUTINE CALL ========================================================
C
      SUBROUTINE INPUTS(FacNam, FacStrt, FacCity, UsrNam, 
     . PFilNam, GIDFNM, RUNFNM, NSITE, NUMNES, GLFNAM, METHOD,
     .  NESSRCNA, NESSRCNUM, FOODFILE,ifodprd)
C
...
C
C-------- NESHAPS Summary output file (.EPA) -----------------------
C
...
C      
 ...
 2212   FORMAT(' Error in specification of NUMBER OF NESHAPS viewers'/
...
C     NOW WE HAVE ALL THE EXP DATA, GO FOR THE AIR DATA (ONLY 1 ALLOWED)
      airNum = GETINT(SETDATA,NLINES,'airNum        ',IS,IZ,IZ,IZ,
     .                  IZ,IZ)
	DO IT = 1, airNUM
	 NAME4 = GETSTR(SETDATA,NLINES,'airNAME       ',IS,IT,IZ,
     .                 IZ,IZ,IZ)     
	   ...
	   END IF
	 END DO
	 airSrcNum = GETINT(SETDATA,NLINES,'airSrcNum     ',IS,I5,IZ,
     .                    IZ,IZ,IZ) 
       DO IU = 1, airSrcNum
	 airSrcName(IU) = GETSTR(SETDATA,NLINES,'airSrcName    ',IS,I5,
     .                  IU,IZ,IZ,IZ)
C     WE CAN SHORT CIRCUIT A STEP HERE AND ASSIGN SrcName's,
C         SO NOW WE HAVE ALL THE INFO NEEDED
       srcName(IU) = airSrcName(IU)   
	 srcNUM = airSrcNum      
       END DO
