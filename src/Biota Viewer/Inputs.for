...
C==== SUBROUTINE CALL ========================================================
C
      SUBROUTINE INPUTS(FacNam, FacStrt, FacCity, UsrNam, 
...
C
      OPEN (UNIT=NSHP,FILE=RUNFNM(1:NRUNAM)//'.BTS',STATUS='UNKNOWN')
C
C-------- Biota Dose Compliance Summary output file (.EPA) -----------------------
C
      OPEN (UNIT=NEPA,FILE=RUNFNM(1:NRUNAM)//'.EPA',STATUS='UNKNOWN')
C
C----- Write run argument values to SHP report file --------------------------
C
      WRITE(NSHP,111) GIDFNM,RUNFNM,NSITE,NUMnes,GLFNAME
...
C      
 ...
 2212   FORMAT(' Error in specification of NUMBER of Biota viewers'/
     .         ' Value given: ',I3,'  Maximum allowed: ',I3)
        NERROR = NERROR + 1
        GO TO 999
      ENDIF
C
C----- Get name of THIS NESHAPS VIEWER GLYPH ----------------------------
	IH = NUMnes
...
C
 ...
 2213   FORMAT(' Error in specification of number of Compliance GLYPHS '
     .        ,\, ' Value must be greater than zero.  Found: ',I3)
        NERROR = NERROR + 1
        GO TO 999
      ENDIF
C
C----- Identify NAMES for the Biota Glyphs -----------------------
C
      DO IT = 1,NESSNUM
...
CCCC
C      First find the Biota Name.   For this application, there should be ONLY ONE
c revision 8/7/2006 to allow for the arrow from the ato generator
      do it = 1, nessnum
      if(seqi(NesSrcNa(IT),'efx',3))NesSrcName = NesSRCNA(it)
	end do
C
C      Now, scan the FUI Section of the GID file for biota info
      efxNum = GETINT(SETDATA,NLINES,'efxNUM        ',IS,IZ,IZ,IZ,
     .                 IZ,IZ)
	 DO IT = 1, efxNUM
	 NAME1 = GETSTR(SETDATA,NLINES,'efxNAME       ',IS,IT,IZ,
     .                 IZ,IZ,IZ)     
	   ...
	   END IF
	 END DO
	 efxSrcNum = GETINT(SETDATA,NLINES,'efxSrcNum     ',IS,I2,IZ,
     .                    IZ,IZ,IZ)
	do inm = 1,efxsrcnum
	 efxSrcName(inm) = GETSTR(SETDATA,NLINES,'efxSrcName    ',IS,I2,
     .                inm,IZ,IZ,IZ)
	  if(seqi(efxsrcname(inm),'usr',3)) then
	    efxusr(inm) = getstr(SETDATA,NLINES,'usrModel      ',IS,
     .                inm,IZ,IZ,IZ,iz)
	    if(seqi(efxusr(inm),'ATO',3)) then
			airname = efxSrcName(inm)
	    else if (seqi(efxusr(inm),'WCF',3)) then
	        rivname = efxSrcName(inm)
	   end if
	  end if
	end do

C     NOW WE HAVE ALL THE EFX DATA, GO FOR air DATA (ONLY 1 ALLOWED)
C
...
C
     ...
     .                  IZ,IZ)
	DO IT = 1, AIRNUM
	 NAME2 = GETSTR(SETDATA,NLINES,'airNAME       ',IS,IT,IZ,
     .                 IZ,IZ,IZ) 
        DO INM = 1,EFXSRCNUM    
	   IF (SEQI(NAME2,efxSrcName(INM),5)) THEN
	     I5 = IT
	     airNAME = NAME2
	   END IF
	  END DO
	END DO
C
C  There may also be surface water sources
C   
	...
	DO IT = 1, rivNUm
     ...
     .                 IZ,IZ,IZ) 
        DO INM = 1,EFXSRCNUM    
	   IF (SEQI(NAME4,EFXSrcName(INM),5)) THEN
	     I5 = IT
	     rivNAME = NAME4
	   END IF
        END DO
...
C
 ...
 1115 FORMAT(' Biota data set sought in GID: ',A16,' lines =',i4)
C
