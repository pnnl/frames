...
Begin VB.Form frmRunAPercent 
   ...
   End
End
Attribute VB_Name = "frmRunAPercent"
...
Private Sub Form_Load()
    ...
    If argc >= 5 Then
        ...
        runcode = App.Path + "\percentile.exe "
        If Dir(runfile) = "" Then
        ...
        If Dir(runcode) = "" Then
            etext = "Unabled to find pecentil.exe - reinstall codes"
            put_val errfile, etext
            put_line errfile
            MsgBox etext
            close_csv errfile
    ...
    If Dir(LTrim(RTrim(RunName)) + ".ERP") <> "" Then
        put_val errfile, "Error running Acutesrc.exe"
        put_line errfile
        close_csv errfile
