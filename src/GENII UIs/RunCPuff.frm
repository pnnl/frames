...
Private Sub Form_Load()
    ...
    If argc >= 5 Then
        ...
        LstData.DomFile = LTrim(RTrim(RunName)) + ".GZ0"
        ...
        'Run Code
        runfile = App.Path + "\chpuff.bat "
        runcode = App.Path + "\pufchron.exe "
        If Dir(runfile) = "" Then
            etext = "Unable to find chpuff.bat - reinstall codes"
        ...
        If Dir(runcode) = "" Then
            etext = "Unabled to find pufchron.exe - reinstall codes"
            put_val errfile, etext
            put_line errfile
            MsgBox etext
            close_csv errfile
    ...
    If Dir(LTrim(RTrim(RunName)) + ".ERP") <> "" Then
        put_val errfile, "Error running PufChron.exe"
        put_line errfile
        close_csv errfile
