         ...
         if (Load_GID(gidf,site,argv[5]))
           {
            ...
            inf=info(gidf,"AquType",site,aqu,1);
            if (strcmpi(inf,"Aquifer") != 0)
            {
              AQUSRC = 0;
              vadpoly = GetPolygon(gidf,argv[5]);     // not working currently
              sprintf(ffile,"%s.gff",argv[1]);
            }
            else
            {
