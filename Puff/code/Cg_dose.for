...
c                  IF( k .LT. 2 ) THEN 
     ...
     &                            + cg_xoq(1,i,j) * qp(n,m,1)
     &                            + cg_xoq(2,i,j) * qp(n,m,2)
     
c                     IF( k .GT. 0 ) THEN
                  cg_dry(n,i,j) = cg_dry(n,i,j) + cg_drq(i,j)*qp(n,m,2)
                  cg_wet(n,i,j) = cg_wet(n,i,j) + cg_weq(i,j)*qp(n,m,2)
     ...
     &                     cg_weq_p(idis,i,j) * qp(n,m,3) * qpf(idis,m)
               ...
               cg_weq(i,j) = 0.0
               DO idis = 1, numpardis(sp(m))
                  cg_xoq_p(idis,i,j) = 0.0
                  cg_drq_p(idis,i,j) = 0.0
                  cg_weq_p(idis,i,j) = 0.0
               ENDDO               
