...
TB_v2::TB_v2(STR_SET setList, char *_mapname, char *_prefix, char *_uidic)
{
...
void TB_v2::KeyListClear(STR_VEC_PTR_VEC keyList)
{
...
int TB_v2::GetKeyList(STR_VEC_MAP setlist)
{
  GetKeyList(setlist, "Chem", "List", "CASID", "CommonName");
  GetKeyList(setlist, prefix, mapname, "Jurisdiction", "Jurisdiction");
  GetKeyList(setlist, prefix, mapname, "UniqueClass", "Jurisdiction");
}

int TB_v2::GetKeyList(STR_VEC_MAP setlist, char *pfix, char *mname, char *vname, char *name)
{
  int i;
  int nkey;
  string cname;
  ...
  Log("Get Key Lists");
  if(0<GetSetName(concat(pfix, mname), setname, blank, setlist))
  {
    nkey = 0;
    if(0 == GetVarDimSize(pid, setname, vname, SetIdx(), &nkey))
      for(i = 0; i<nkey; i++)
      {
        v = new STR_VEC; TrackLocation(v);
        v->push_back(string(prefix));
        if(0 == GetString(pid, setname, vname, blank, SetIdx(i+1), dummy))
        {
      ...
      Log("ERROR getting variable dim size");
  }
  // reoder list
  KeyListClear();
...
int TB_v2::GetMappingOfSectionParameters(STR_SET dicList)
{
  int i;
  int j;
  int ndic;
...
int TB_v2::ReadDatasets(Glyph *g)  
{
  int j;
  int okey;
  int ErrorCode;
...
int TB_v2::ReadProperties(Glyph *g, char *dicprefix, int *v2idx, int *v1idx, int depidx)
{
  int i;
  int dim;
  int dep;
  ...
  for(ivar = VarMap.begin(); ivar!= VarMap.end(); ++ivar)
  {
    ...
    if(it!= g->osetlist.end())
    {
      ...
      {
        dep = pm->GetDependentValueIndex(dicprefix, g->osetlist, false, 1);
        if(dep>=0)
          inpidx[depidx] = dep;
...
int TB_v2::WriteDatasets(Glyph *g)
{
  int i;
  int n;
  int vct;
...
void TB_v2::WriteProperties(Glyph *g, char *dicprefix, int *v1idx, int *v2idx, int depidx)
{
  int i;
  int dep;
  int ktype;
  ...
  for(ivar = VarMap.begin(); ivar!= VarMap.end(); ++ivar)
  {
    ...
    if(it!= g->osetlist.end())
    {
      ...
      if(p!= NULL)
      {
        ...
        {
          dep = pm->GetDependentValueIndex(dicprefix, g->osetlist, true, 1);
          if(dep>0)
            outidx[depidx] = dep;
